    ...
    try:
        ...
        if main_codes:
            ...
            if not df_main_data.empty:
                df_main_data[["main_contract_amount", "main_total_collected"]] = df_main_data[
                    ["main_contract_amount", "main_total_collected"]
                ].astype(float)
                df_main_data["actual_main_code"] = df_main_data["actual_main_code"].astype(str).str.strip()

        v_cols = [
            "total_paid",
            "total_invoiced_from_sub",
            "main_contract_amount",
            "main_total_collected",
        ]
...
def validate_sub_payment_risk(sub_biz_code: str, apply_amount: float, conn=None) -> tuple[bool, str]:
    ...
    try:
        ...
        cur.execute(
            "SELECT * FROM biz_sub_contracts WHERE biz_code = %s AND deleted_at IS NULL FOR UPDATE",
            (sub_biz_code,),
        )
        ...
        cur.execute(
            "SELECT SUM(payment_amount) as paid FROM biz_outbound_payments WHERE sub_contract_code = %s AND deleted_at IS NULL",
            (sub_biz_code,),
        )
        ...
        cur.execute(
            "SELECT * FROM biz_main_contracts WHERE biz_code = %s AND deleted_at IS NULL",
            (ebm_code,),
        )
        ...
        cur.execute(
            "SELECT SUM(collected_amount) as coll FROM biz_collections WHERE main_contract_code = %s AND deleted_at IS NULL",
            (ebm_code,),
        )
        ...
        if future_ratio > (main_ratio + 0.001):
            return (
                False,
                f"🔴 安全拦截：背靠背条款限制！主合同当前收款率仅 {main_ratio:.1%}，本次付款将使分包支付率达 {future_ratio:.1%}，突破红线！",
            )
