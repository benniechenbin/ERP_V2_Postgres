...
def update_biz_code_cascade(old_code, new_code, table_name):
    ...
    try:
        ...
        with conn.cursor() as cur:  # 🟢 必须创建游标
            # 1. 改主表
            cur.execute(
                f'UPDATE "{table_name}" SET biz_code = %s WHERE biz_code = %s',
                (new_code, old_code),
            )

            # 2. 改周边所有的子表
            cur.execute(
                "UPDATE biz_payment_plans SET main_contract_code = %s WHERE main_contract_code = %s",
                (new_code, old_code),
            )
            cur.execute(
                "UPDATE biz_invoices SET main_contract_code = %s WHERE main_contract_code = %s",
                (new_code, old_code),
            )
            cur.execute(
                "UPDATE biz_collections SET main_contract_code = %s WHERE main_contract_code = %s",
                (new_code, old_code),
            )
            cur.execute(
                "UPDATE sys_attachments SET biz_code = %s WHERE biz_code = %s",
                (new_code, old_code),
            )
