...
def get_all_data_tables():
    ...
    try:
        ...
        if settings.DB_TYPE == "sqlite":
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND (name LIKE 'data_%' OR name LIKE 'biz_%')"
            )
...
def _create_dynamic_business_tables(cursor):
    ...
    for config in models.values():
        ...
        if not table_name or not field_meta:
            continue
        ...
        for field_key, meta in field_meta.items():
            ...
            if field_type == "money":
                col_def = f"{field_key} NUMERIC(15,2) DEFAULT 0.00"
            elif field_type == "percent":
                col_def = f"{field_key} REAL DEFAULT 0"
            elif field_type == "date":
                col_def = f"{field_key} DATE"
            elif field_type == "int":
                col_def = f"{field_key} INTEGER DEFAULT 0"
            else:
                col_def = f"{field_key} VARCHAR(255)"
        ...
        for field_key, meta in field_meta.items():
            if field_key not in existing_columns:
                field_type = meta.get("type", "text")
                if field_type == "money":
                    alter_type = "NUMERIC(15,2) DEFAULT 0.00"
                elif field_type == "date":
                    alter_type = "DATE"
                else:
