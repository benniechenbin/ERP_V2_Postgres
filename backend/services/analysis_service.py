...
def calculate_overall_margin() -> dict:
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT COALESCE(SUM(contract_amount::numeric), 0) FROM biz_main_contracts WHERE deleted_at IS NULL"
        )
...
def _flatten_extra_props(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "extra_props" in df.columns:
        ...
        props_df = (
            df["extra_props"]
            .apply(lambda x: x if isinstance(x, dict) else (json.loads(x) if isinstance(x, str) else {}))
            .apply(pd.Series)
        )
