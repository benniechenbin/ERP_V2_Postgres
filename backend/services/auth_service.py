...
def verify_user_login(username: str, password: str) -> tuple[bool, str, dict]:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, username, password_hash, role, status FROM sys_users WHERE username = %s",
                (username,),
            )
            user = cur.fetchone()
            ...
            if check_password_hash(user[2], password):
                # 登录成功，顺手更新最后登录时间
                cur.execute(
                    "UPDATE sys_users SET last_login_at = CURRENT_TIMESTAMP WHERE id = %s",
                    (user[0],),
                )
                conn.commit()
...
def toggle_user_status(username: str, is_active: bool) -> tuple[bool, str]:
    ...
    success, msg = execute_raw_sql("UPDATE sys_users SET status = %s WHERE username = %s", (new_status, username))
