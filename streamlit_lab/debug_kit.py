...
def execute_debug_logic(current_db_path=None):
    ...
    with tabs[0]:
        ...
        st.caption(
            "⚠️ 警告：此区域用于紧急修改表结构与映射规则。修改后保存，系统底层的引擎会自动将 label 对齐到 column_mapping 中。"
        )
    ...
    with tabs[1]:
        ...
        SQL_TEMPLATES = {
            ...
            "📑 查看表结构 (列定义)": (
                ...
                if is_sqlite
                else f"SELECT column_name, data_type, character_maximum_length, column_default FROM information_schema.columns WHERE table_name = '{default_table}';"
            ),
