...
def run(df, conn):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                for idx, row in df_plans.iterrows():
                    ...
                    if not milestone_name or milestone_name == "nan":
                        continue
    ...
    def contract_form_dialog(existing_data=None):
        ...
        if result:
            ...
            success, msg = crud_base.upsert_dynamic_record(
                "main_contract", result, record_id=target_id, operator_name=current_user
            )
    ...
    def yearly_archive_dialog():
        st.warning(
            "⚠️ 警告：此操作会将系统内所有【已计提】的合同和项目进行归档（软删除）。\n\n请务必确保本年度所有账务已核对完毕！"
        )
        confirm = st.text_input("请输入 '确认结转' 以继续执行：")
        if st.button(
            "🚨 确认执行结转",
            type="primary",
            disabled=(confirm != "确认结转"),
            width="stretch",
        ):
            # 🟢 1. 提取当前操作人
            current_user = st.session_state.get("user_name", "System")
    ...
    if not df_main.empty:
        ...
        with col1:
            st.metric(
                "📦 总盘子 (合同总额)",
                f"¥ {total_amount:,.2f}",
                delta=f"包含 {contract_count} 份主合同",
                delta_color="off",
            )  # off 代表显示中性灰色
        with col2:
            st.metric(
                ...
                delta=f"总回款率: {collection_rate:.1f}%",
            )  # 默认 normal 为绿色
        with col3:
            st.metric(
                "📝 待收款 (剩余未到账)",
                f"¥ {total_uncollected:,.2f}",
                delta=f"资金缺口占比: {uncollected_rate:.1f}%",
                delta_color="inverse",
            )  # inverse 代表红色预警
        with col4:
            st.metric(
    ...
    if df_main.empty:
        ...
        with col_table:
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    "项目阶段": st.column_config.SelectboxColumn(
                        "📌 当前阶段 (双击修改)",
                        help="修改后自动保存",
                        options=stage_options,
                        required=True,
                    ),
                    "回款进度(%)": st.column_config.ProgressColumn(
                        "回款进度(%)", format="%.2f %%", min_value=0.0, max_value=100.0
                    ),
            ...
            if not edited_df.equals(df_display):
                ...
                for i in range(len(df_display)):
                    if df_display.iloc[i]["项目阶段"] != edited_df.iloc[i]["项目阶段"]:
                        ...
                        execute_raw_sql(
                            f'UPDATE "{target_table}" SET project_stage = %s WHERE biz_code = %s',
                            (new_stage, biz_code),
                        )
                        changes_detected = True
                if changes_detected:
                    if ui and hasattr(ui, "show_toast_success"):
                        ui.show_toast_success("✅ 项目阶段已同步至数据库！")
                    trigger_refresh()
                    st.rerun()
        ...
        with col_form:
            ...
            checked_rows = edited_df[edited_df["☑️"]]
            if not checked_rows.empty:
                auto_selected_biz_code = checked_rows.iloc[-1]["合同编号"]

            col_select, col_edit = st.columns([8, 2])
            with col_select:
                contract_options = df_main.apply(
                    lambda row: f"{row['biz_code']} | {row['project_name']}", axis=1
                ).tolist()
            ...
            current_contract_amount = float(
                df_main[df_main["biz_code"] == selected_biz_code]["contract_amount"].iloc[0]
            )
            load_payment_plans(selected_biz_code)

            tab_flow, tab_history, tab_plan, tab_audit = st.tabs(
                ["⚡ 录入流水", "📜 财务明细", "📝 收款计划", "🛡️ 高级与审计"]
            )
            ...
            with tab_flow:
                ...
                with st.form(key="flow_form", clear_on_submit=True):
                    ...
                    input_stage = st.text_input(
                        "🎯 对应业务节点/阶段",
                        value=suggested_stage,
                        help="系统已自动提取当前阶段，您可根据实际情况微调（如：方案一期款）",
                    )
                    ...
                    if submit_btn:
                        ...
                        if success:
                            # 🟢 同步推演：自动把主合同的阶段改过去！
                            if selected_node_name != "通用/无特定节点":
                                target_table = cfg.get_model_config("main_contract").get(
                                    "table_name", "biz_main_contracts"
                                )
                                execute_raw_sql(
                                    f'UPDATE "{target_table}" SET project_stage = %s WHERE biz_code = %s',
                                    (selected_node_name, selected_biz_code),
                                )
            ...
            with tab_history:
                ...
                with sub_tab_coll:
                    ...
                    if not df_coll.empty:
                        ...
                        edited_coll = st.data_editor(
                            ...
                            column_config={
                                "🔴 作废": st.column_config.CheckboxColumn(
                                    "🔴 作废",
                                    default=False,
                                    help="勾选后点击下方按钮作废",
                                ),
                        ...
                        # 侦测选中项
                        selected_colls = edited_coll[edited_coll["🔴 作废"]]
                ...
                with sub_tab_inv:
                    ...
                    if not df_inv.empty:
                        ...
                        selected_invs = edited_inv[edited_inv["🔴 作废"]]
            ...
            with tab_plan:
                ...
                if st.button("💾 确认保存/覆盖计划", width="stretch", type="primary"):
                    current_user = st.session_state.get("user_name", "System")
                    success, msg = save_payment_plans(
                        selected_biz_code,
                        edited_plans,
                        current_user,
            ...
            with tab_audit:
                ...
                is_accrued = (
                    df_main[df_main["biz_code"] == selected_biz_code].get("is_provisioned", pd.Series(["否"])).iloc[0]
                    == "是"
                )
