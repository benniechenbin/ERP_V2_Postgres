...
def show_ai_search_dialog(keyword, initial_df):
    ...
    q = st.text_area(
        "🗣️ 设定 AI 分析指令：",
        value="分析这些项目的回款风险，找出回款率低于30%的重点项目，并给出催款建议。",
        height=100,
    )
...
if not df_urgent.empty:
    ...
    st.error(
        f"🚨 **资金红绿灯**：未来 30 天内共有 **{len(df_urgent)}** 笔款项即将到期/已逾期，涉及总金额 **¥ {total_urgent:,.2f}**，请重点督办！"
    )
...
if search_kw:
    mask = df_view["project_name"].astype(str).str.contains(search_kw, case=False) | df_view["biz_code"].astype(
        str
    ).str.contains(search_kw, case=False)
