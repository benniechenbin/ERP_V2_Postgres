...
def contract_form_dialog(existing_data=None):
    ...
    with c_btn:
        ...
        if st.button("✨ 一键 AI 提取", type="primary", disabled=not ai_ready, width="stretch"):
            ...
            with st.spinner("🧠 AI 正在极速阅读合同条款，请稍候..."):
                ai_results = extract_contract_elements(uploaded_files[0], "main_contract", dispatcher=ai_engine)
    ...
    for k, v in list(current_data.items()):
        if pd.isna(v):
            ...
            is_forced_text = (
                f_type in ["text", "select"]
                or field_meta.get(k, {}).get("readonly")
                or field_meta.get(k, {}).get("is_virtual")
            )
                    ...
                    edited_coll = st.data_editor(
                        ...
                        column_config={
                            "🔴 作废": st.column_config.CheckboxColumn(
                                "🔴 作废", default=False, help="勾选后点击下方按钮作废"
                            ),
