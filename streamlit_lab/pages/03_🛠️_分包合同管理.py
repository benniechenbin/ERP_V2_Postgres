        ...
        with c_btn:
            ...
            if st.button(
                ...
                with st.spinner("🧠 AI 正在极速阅读分包条款，请稍候..."):
                    ai_results = extract_contract_elements(uploaded_files[0], "sub_contract", dispatcher=ai_engine)
...
if not df_sub.empty:
    ...
    total_inv_rec = (
        df_sub["total_invoiced_from_sub"].astype(float).sum() if "total_invoiced_from_sub" in df_sub.columns else 0.0
    )
    ...
    with col4:
        ...
        st.metric(
            "⚠️ 欠票敞口 (付>票)",
            f"¥ {missing_invoices:,.2f}",
            delta=warn_text,
            delta_color=warn_color,
        )
...
if df_sub.empty:
    ...
    with col_table:
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "背靠背": st.column_config.CheckboxColumn(
                    "背靠背条款", help="打勾代表需等甲方付款后才付款", disabled=True
                ),
                ...
                "本期应付": st.column_config.NumberColumn(
                    "本期应付(背靠背)", disabled=True, format="¥ %.2f"
                ),  # 🟢 增加这行列配置！
            },
            width="stretch",
            hide_index=True,
    ...
    with col_form:
        ...
        with col_select:
            contract_options = df_sub.apply(
                lambda row: f"{row['biz_code']} | {row['sub_company_name']}", axis=1
            ).tolist()
            default_index = 0
        ...
        with tab_flow:
            ...
            with st.form(key="sub_flow_form", clear_on_submit=True):
                ...
                if "收票" in action_type:
                    ...
                    inv_type_options = (
                        cfg.get_field_meta("sub_contract").get("tax_rate", {}).get("options", ["3%", "6%", "9%"])
                    )
