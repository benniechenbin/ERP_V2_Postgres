...
k1.metric(
    f"{analysis_year}年 新签合同额",
    f"¥ {new_contract_sum / 10000:,.1f} 万",
    f"{len(df_new)} 个项目",
)
k2.metric(
    "本年新签回款率",
    f"{(new_collection_sum / new_contract_sum * 100):.1f}%" if new_contract_sum else "0.0%",
    f"回款: ¥{new_collection_sum / 10000:.1f}万",
    delta_color="off",
)
k3.metric(
    "往年结转欠款",
    f"¥ {carry_debt_sum / 10000:,.1f} 万",
    "存量风险",
    delta_color="inverse",
)
k4.metric(
    "全盘总应收账款",
    f"¥ {total_debt / 10000:,.1f} 万",
    "需催收总额",
    delta_color="inverse",
)
        ...
        if not df_carry_debt.empty:
            st.dataframe(
                ...
                column_config={
                    ...
                    "uncollected_rate": st.column_config.ProgressColumn(
                        "欠款比例", format="%.1f%%", min_value=0, max_value=100
                    ),
                },
                hide_index=True,
                width="stretch",
...
with tab2:
    if not df_new.empty:
        ...
        df_melt = by_manager.melt(
            id_vars="manager_safe",
            value_vars=["val_contract", "val_collection"],
            var_name="类型",
            value_name="金额",
        )
        # 汉化图例
...
with st.container(border=True):
    ...
    if st.button("✨ 一键生成经营诊断书", type="primary"):
        # 🟢 预留给二期工程的解耦占位提示
        st.info(
            "💡 架构解耦准备中：此处宏观数据的整合逻辑及 CFO 提示词，将在二期工程中抽离至独立的 `prompts.py` 模块。"
        )
        # 实际功能暂时悬空
