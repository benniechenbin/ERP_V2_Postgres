...
def render_enterprise_form(mode="edit", initial_data=None, model_name="enterprise", target_table=None):
    ...
    # 获取正确的 biz_code
    if mode == "add":
        new_code = crud.generate_biz_code(table_name=target_table, prefix_char="ENT")
    ...
    if result:
        ...
        # 调用后端通用写入引擎
        success, msg = crud.upsert_dynamic_record(model_name=model_name, data_dict=result, record_id=target_id)
...
model_options = [m for m, _ in all_models.items() if "project" not in m.lower()]
...
selected_model = st.selectbox(
    ...
    model_options,
    index=default_idx,
    # 🟢 核心魔法：格式化展示。拿到英文 key (m)，去字典里翻出对应的 model_label
    format_func=lambda m: all_models[m].get("model_label", m),
)
