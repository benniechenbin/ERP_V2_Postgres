...
with tab_users:
    ...
    # 获取用户列表
    success, df_users = execute_raw_sql(
        "SELECT id, username, role, status, last_login_at, created_at FROM sys_users ORDER BY id ASC"
    )
    ...
    if success and not df_users.empty:
        ...
        edited_users = st.data_editor(
            ...
            column_config={
                ...
                "role": st.column_config.SelectboxColumn(
                    "角色", options=["普通员工", "部门经理", "财务专员", "系统管理员"]
                ),
        ...
        # 密码重置区
        selected_users = edited_users[edited_users["☑️ 选中"]]
            ...
            for m_name, m_cfg in config_models.items():
                ...
                try:
                    ...
                    name_col = (
                        ...
                        else ("sub_company_name" if "sub_company_name" in cols else "biz_code")
                    )
