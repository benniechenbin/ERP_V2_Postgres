...
def setup_business_data():
    ...
    res2, _ = upsert_dynamic_record("main_contract", {
    ...
    res4, _ = upsert_dynamic_record("sub_contract", {
    ...
    # 记录生成的 ID 供清理
    df_m = pd.read_sql_query("SELECT id FROM biz_main_contracts WHERE biz_code LIKE 'TEST-FORMULA-%'", db_engine.get_connection())
    main_ids = df_m["id"].tolist()
    
    df_s = pd.read_sql_query("SELECT id FROM biz_sub_contracts WHERE biz_code LIKE 'TEST-FORMULA-%'", db_engine.get_connection())
