...
from backend.database.crud_finance import check_main_contract_clearance, submit_sub_payment, mark_project_as_accrued
...
def setup_risk_data():
    ...
    res1, _ = upsert_dynamic_record("main_contract", {
    ...
    res2, _ = upsert_dynamic_record("sub_contract", {
    ...
    res3, _ = upsert_dynamic_record("sub_contract", {
    ...
    df_m = pd.read_sql_query("SELECT id FROM biz_main_contracts WHERE biz_code LIKE 'TEST-RISK-%'", db_engine.get_connection())
    main_ids = df_m["id"].tolist()
    
    df_s = pd.read_sql_query("SELECT id FROM biz_sub_contracts WHERE biz_code LIKE 'TEST-RISK-%'", db_engine.get_connection())
...
def test_main_contract_clearance_accrual(setup_risk_data):
    ...
    # 模拟支付完分包款 (结清分包)
    # 对分包 1 (200万) 支付 200万
    success_pay1, msg_pay1 = submit_sub_payment(sub_biz_code="TEST-RISK-S01", payment_amount=2000000.0, operator="测试财务", payment_date="2026-03-22")
    ...
    # 对分包 2 (100万) 支付 100万
    success_pay2, msg_pay2 = submit_sub_payment(sub_biz_code="TEST-RISK-S02", payment_amount=1000000.0, operator="测试财务", payment_date="2026-03-22")
